...
def getName():
    player1.namePlayer = str(input("Enter the player's Ursername 1:"))
    player2.namePlayer = str(input("Enter the player's Username 2:"))
    if player1.namePlayer.strip() == "" or player1.name.strip() == "":
...
while isPartyFinish == False:
    if len(player1.listCardsPlayer) ==0 or len(player2.listCardsPlayer) == 0:
        ...
        for player in listPlayer:
           if len(player.listCardsPlayer) == 0:
              print(f'END OF THE GAME. Winner: {player1.namePlayer} ')
        break
    for player in listPlayer:
    ...
    for joueur in listPlayer:
      ...
      if len(listCardpossibletoPlay) == 0:
         ...
         print(f"{n+1}-Piocher")
         print()
         print("Ton choix (seul l'index est nécessaire):")
         while True:
            try:
               choice = int(input())
